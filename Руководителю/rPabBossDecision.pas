...
procedure Report_Pab_BossDecision(date1:TDateTime;date2:TDateTime;user_id,ceh_id:integer);
...
begin
  ...
  sql := sql +' order by isnull(boss_decision,''я''),boss_decision,ceh,uchastok,date';
  ...
  e.Open(path+'templates\pab\отчет по выбранным решениям.xlt');
  e.Show;
  ...
  data := VarArrayCreate([0,datalen,0,11],varVariant);
  ...
  with AQuery do
  begin
    typ := nz(FieldByName('boss_decision').Value,'');
    if typ<>last_typ then
    begin
      if typ='' then
        data[rowid,0] := 'решение еще не принято'
      else
        data[rowid,0] := typ;
      last_typ := typ;
      e.Range('A',9+rowid,'K',9+rowid).Merge;
      e.Range('A',9+rowid,'K',9+rowid).Font.Bold := true;
      inc(rowid);
    end;
    ...
    data[rowid,6] := nz(FieldByName('type').Value,'');
    data[rowid,7] := nz(FieldByName('fatal_effect').Value,'');
    data[rowid,8] := nz(FieldByName('work_kind').Value,'');
    data[rowid,9] := nz(FieldByName('possible_effect').Value,'');
    data[rowid,10] := nz(FieldByName('boss_name').Value,'');
  ...
  e.Range('A',9,'K',9+datalen).Value := data;
  VarClear(data);
  if rowid<datalen+1 then
  begin
    e.Range('A',9+rowid,'K',9+datalen).Delete;
  end;
  e.Free;
end;
